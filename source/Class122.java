...
final class Class122 implements Comparator {
   ...
   static final void method452(ByteBuf buf, int plane, int x, int y, int chunkX, int chunkY, int shapeOffset) {
      int attribute;
      if(x >= 0 && x < 104 && y >= 0 && y < 104) {
         ...
         while(true) {
            ...
            if(attribute == 0) {
               if(plane == 0) {
                  Class39.tileHeights[0][x][y] = -Character.calculateTileHeight(chunkX + 932731 + x, chunkY + 556238 + y) * 8;
               } else {
