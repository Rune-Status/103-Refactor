...
public class ByteBuf extends Node {
	...
	public int getMEMedium() {
		this.position += 3;
		return ((this.payload[this.position - 2] & 0xff) << 16) + ((this.payload[this.position - 3] & 0xff) << 8)
	...
	public void putVarLen(int var1) {
	...
	public int getMedium() {
		this.position += 3;
		return (this.payload[this.position - 1] & 0xff) + ((this.payload[this.position - 3] & 0xff) << 16)
				+ ((this.payload[this.position - 2] & 0xff) << 8);
	}

	public int getInt() {
		this.position += 4;
		return (this.payload[this.position - 1] & 0xff) + ((this.payload[this.position - 4] & 0xff) << 24)
				+ ((this.payload[this.position - 3] & 0xff) << 16) + ((this.payload[this.position - 2] & 0xff) << 8);
	}
	...
	public String getCESU8() {
		...
		if (var1 != 0) {
			...
			int size = this.getVarLen();
			if (this.position + size > this.payload.length) {
				throw new IllegalStateException("");
			} else {
				String string = Class46.decodeCESU8(this.payload, this.position, size);
				this.position += size;
				return string;
			}
		}
	...
	public void encryptXTEA(int[] keys, int offset, int length) {
		...
		for (int var9 = 0; var9 < var8; var9++) {
			...
			for (int var10 = 32; var10-- > 0; var5 += (var6 << 4 ^ var6 >>> 5) + var6
					^ keys[var7 >>> 11 & 0x3] + var7) {
	...
	public int getLEInt() {
		this.position += 4;
		return ((this.payload[this.position - 2] & 0xff) << 16) + ((this.payload[this.position - 1] & 0xff) << 24)
				+ ((this.payload[this.position - 3] & 0xff) << 8) + (this.payload[this.position - 4] & 0xff);
	}
	...
	public int getIMEInt() {
		this.position += 4;
		return (this.payload[this.position - 2] & 0xff) + ((this.payload[this.position - 3] & 0xff) << 24)
				+ ((this.payload[this.position - 4] & 0xff) << 16) + ((this.payload[this.position - 1] & 0xff) << 8);
	}
	...
	static final int method637(int var0, int var1, int var2) {
		int var3 = 256 - var2;
		return ((var0 & 0xff00) * var3 + (var1 & 0xff00) * var2 & 0xff0000)
				+ ((var1 & 0xff00ff) * var2 + (var0 & 0xff00ff) * var3 & 0xff00ff00) >> 8;
	}
